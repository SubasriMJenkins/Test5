...
public class ViewElog {
		...
		public void clickViewHomework() throws InterruptedException{
			...
			if (driver.getTitle().equals("Result Analysis | Admission Status Approval")) {
				...
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//a[contains(text(),'View eLog')]"))).click();
		...
		public void selectGrade() throws InterruptedException{
			...
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//mat-option[@id='mat-option-7']"))).click(); // 8
							...
							for (int i = 0; i < 2; i++) {
								...
								WebElement grade = gradeList.get(i);
									...
									if(grade.isDisplayed()) {
										...
										String postedDate = dateElement.getText();
