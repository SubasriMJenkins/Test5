...
public class TeacherRole {
	...
	public void clickViewCircular() throws InterruptedException{
		...
		if (driver.getTitle().equals("Result Analysis | Admission Status Approval")) {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//a[contains(text(),'View Circular')]"))).click();
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[contains(text(),'View Circular')]"))).click();
		}
	}
	
	@Test(priority=4)

	public void displayTodayCircularRecords() throws InterruptedException{
		...
		for(int eachCard=0;eachCard<cardsList.size();eachCard++) {
            ...
            if (cardDateFormatted.equals(formattedDate)) {
            	...
            	System.out.println("No circular has posted today");
            	break;
            }
		}
	}
