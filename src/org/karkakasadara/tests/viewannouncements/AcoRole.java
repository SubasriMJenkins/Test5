...
public class AcoRole {
	...
	public void clickViewCircular() throws InterruptedException{
		...
		if (driver.getTitle().equals("Result Analysis | Admission Status Approval")) {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//a[contains(text(),'View Announcements')]"))).click();
	...
	public void displayTodayAnnouncementRecords() throws InterruptedException{
		...
		List<WebElement> cardsList= wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//div[contains(@class,'flex flex-col h-auto shadow rounded-2xl overflow-hidden bg-card px-6 pb-2 ng-star-inserted')]")));
		...
		for(int eachCard=0;eachCard<cardsList.size();eachCard++) {
            ...
            if (cardDateFormatted.equals(formattedDate)) {
                ...
                WebElement messageElement = driver.findElement(By.xpath("(//div[contains(@class,'flex flex-col h-auto shadow rounded-2xl overflow-hidden bg-card px-6 pb-2 ng-star-inserted')])[" + (eachCard + 1) + "]"));
