...
public class KgClassTeacher {
	...
	WebElement gradeElement=wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(@class,'ng-star-inserted')]//b")));
    ...
    for (int card=0;card<eachCard.size()-1; card++) {
        try {
            ...
            WebElement postedMessage = driver.findElement(By.xpath("(//div[contains(@class,'flex flex-col flex-auto p-6 bg-card shadow rounded-2xl overflow-hidden pt-6 px-5 pb-6 w-full pt-6 sm:col-span-6 mat-elevation-z0')])[" + (card+1) + "]"));
    ...
    WebElement noteCard = eachCard.getLast();
    if(noteCard.isDisplayed()) {
    	
    	String note= noteCard.getText();
